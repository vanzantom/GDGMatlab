function [x,y]=square_twosub(bs,s)
%SQUAREG   Gives geometry data for the squareg PDE model
%
%   NE=square_twosub gives the number of boundary segment
%
%   D=square_twosub(BS) gives a matrix with one column for each boundary segment
%   specified in BS.
...
%
%   [X,Y]=square_twosub(BS,S) gives coordinates of boundary points. BS specifies the
...
ii=find(bs==5);
if length(ii)
x(ii)=interp1([d(1,5),d(2,5)],[1/2 1],s(ii));
y(ii)=interp1([d(1,5),d(2,5)],[1 1],s(ii));
end
% boundary segment 6
ii=find(bs==6);
if length(ii)
x(ii)=interp1([d(1,6),d(2,6)],[1 1],s(ii));
y(ii)=interp1([d(1,6),d(2,6)],[1 0],s(ii));
end
% boundary segment 7
ii=find(bs==7);
if length(ii)
x(ii)=interp1([d(1,7),d(2,7)],[1 1/2],s(ii));
