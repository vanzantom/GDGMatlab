...
% para: structure containing structure containing several parameters such as the basis_type(type
%       of FEM/DG space, e.g linear,quadratic etc..), order of Gauss
%       quadrature and (eventual)
%       DG penalization coefficient.
% method: string that specifies if we are using FEM or DG. If method=='DG' then it computes also the contribution on edges.
...
function [error_L2,error_H1,error_DG]=compute_errors(u,uex,P,T,Tb,Eb,para,method)
...
for n=1:number_of_elements % Loop over the triangles.
    ...
    [Gauss_nodes,Gauss_weights]=generate_Gauss_2D(vertices,para.order_Gauss); % create Gauss nodes on the element. 1 baricentri 2 order accuracy.
...
for n=1:number_of_edges % Loop over the edges
    ...
    [Gauss_nodes,Gauss_weights]=generate_Gauss_1D(P(:,vertex1),P(:,vertex2),para.order_Gauss);
    ...
    mu=para.alpha_coef/edge_length;
    if(T_right~=-1)
        ...
        for k=1:size(Gauss_nodes,2) % for each gauss node
            ...
            uapp_minus=0;
            for alpha=1:number_of_local_basis_trial %loop over trial functions on the element for trial FE space.
                uapp_plus=uapp_plus +(u_app_plus(alpha))*FE_local_basis_2D(Gauss_nodes(1,k),Gauss_nodes(2,k),vertices_plus,para.basis_type,alpha,0,0);
                uapp_minus=uapp_minus +(u_app_minus(alpha))*FE_local_basis_2D(Gauss_nodes(1,k),Gauss_nodes(2,k),vertices_minus,para.basis_type,alpha,0,0);
            end
