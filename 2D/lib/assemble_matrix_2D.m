...
%=== Create sparse matrix and define parameters.%create sparse matrix
...
count=matrixsize1*matrixsize2;
Ivec=zeros(number_of_elements*number_of_local_basis_trial*number_of_local_basis_test,1);
Jvec=zeros(number_of_elements*number_of_local_basis_trial*number_of_local_basis_test,1);
Xvec=zeros(number_of_elements*number_of_local_basis_trial*number_of_local_basis_test,1);
count=0;
Gauss_nodes=zeros(order_Gauss^2,2);
Gauss_weights=zeros(1,order_Gauss^2);
%=== Loop to assembly
for n=1:number_of_elements % Loop over the triangles.
    ...
    for alpha=1:number_of_local_basis_trial %loop over trial functions on the element for trial FE space.
        for beta=1:number_of_local_basis_test %loop over trial functions on the element for test FE space.
            ...
            count=count+1;
            Ivec(count)=Tb_test(beta,n);
            Jvec(count)=Tb_trial(alpha,n);
            Xvec(count)=int_value;
        end
    end
    
end
A = sparse (Ivec(1:count),Jvec(1:count),Xvec(1:count),matrixsize1,matrixsize2) ;
end