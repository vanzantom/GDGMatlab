function A=assemble_matrix_edges1D(coe_fun,P,T,Tb_trial,Tb_test,Eb,matrixsize1,matrixsize2,basis_type_trial,basis_type_test,penalty)
  ...
  for n=1:number_of_edges   %cycle over the interior edges.
    ...
    for alpha=1:number_of_local_basis_trial % Contribution from term involving basis on same element.
        for beta=1:number_of_local_basis_test %from reference-> local -> global assembly.
            ...
            A(Tb_test(beta,T1),Tb_trial(alpha,T1))=A(Tb_test(beta,T1),Tb_trial(alpha,T1))+penalty/2*stabilizationIPH(coe_fun,vertices1,vertices1,vertex,basis_type_trial,beta,0,basis_type_test,alpha,0)-(1/(2*penalty))*stabilizationIPH(coe_fun,vertices1,vertices1,vertex,basis_type_trial,beta,1,basis_type_test,alpha,1);
            A(Tb_test(beta,T2),Tb_trial(alpha,T1))=A(Tb_test(beta,T2),Tb_trial(alpha,T1))-penalty/2*stabilizationIPH(coe_fun,vertices2,vertices1,vertex,basis_type_trial,beta,0,basis_type_test,alpha,0)+(1/(2*penalty))*stabilizationIPH(coe_fun,vertices2,vertices1,vertex,basis_type_trial,beta,1,basis_type_test,alpha,1);
            A(Tb_test(beta,T1),Tb_trial(alpha,T2))=A(Tb_test(beta,T1),Tb_trial(alpha,T2))-penalty/2*stabilizationIPH(coe_fun,vertices1,vertices2,vertex,basis_type_trial,beta,0,basis_type_test,alpha,0)+(1/(2*penalty))*stabilizationIPH(coe_fun,vertices1,vertices2,vertex,basis_type_trial,beta,1,basis_type_test,alpha,1);
            A(Tb_test(beta,T2),Tb_trial(alpha,T2))=A(Tb_test(beta,T2),Tb_trial(alpha,T2))+penalty/2*stabilizationIPH(coe_fun,vertices2,vertices2,vertex,basis_type_trial,beta,0,basis_type_test,alpha,0)-(1/(2*penalty))*stabilizationIPH(coe_fun,vertices2,vertices2,vertex,basis_type_trial,beta,1,basis_type_test,alpha,1);
            % symmetric blocks
...
% I treat the boundary terms in penalization way.
    ...
    for alpha=1:number_of_local_basis_trial % Contribution from term involving basis on same element.
        for beta=1:number_of_local_basis_test %from reference-> local -> global assembly.
            %penalization \mu block
            A(Tb_test(beta,T1),Tb_trial(alpha,T1))=A(Tb_test(beta,T1),Tb_trial(alpha,T1))+penalty/2*stabilizationIPH(coe_fun,vertices1,vertices1,vertex,basis_type_trial,beta,0,basis_type_test,alpha,0);
            A(Tb_test(beta,T2),Tb_trial(alpha,T2))=A(Tb_test(beta,T2),Tb_trial(alpha,T2))+penalty/2*stabilizationIPH(coe_fun,vertices2,vertices2,vertex2,basis_type_trial,beta,0,basis_type_test,alpha,0);
            %symmetry block
