%-------------------------------------------------------------------------
% example_IP is an example of script using Poisson_solver_1DIP() function to solve
% a Poisson problem on the interval (a,b) with Dirichlet/Neumann BC with
% linear or quadratic DG spaces using Interior penalty method.
...
data.Neumann_fun=@(x) 2*pi*cos(2*pi*x); %DO NOT USE NEUMANN
...
plt=0;
%=================================
%   Linear DG Interior Penalty
%=================================
basis_type=101;
for i=1:length(hh)
geo.h=hh(i); %mesh size
penalty=alpha/geo.h; %penalty term 
[u,P,T,Pb,Tb]=Poisson_solver_1DIP(geo,basis_type,data,penalty);
...
loglog(hh,hh.^1,'r')
%=================================
%   Quadratic DG Interior Penalty
%=================================
basis_type=102;
for i=1:length(hh)
geo.h=hh(i);
penalty=alpha/geo.h;
[u2order,P,T,Pb,Tb]=Poisson_solver_1DIP(geo,basis_type,data,penalty);
